...
Xtest = [ones(te,1) rand(te, n-1)];
ytest = (Xtest*u).^2 + randn(te,1)*sigma;
for r = 1:repeats
   ...
   w1 = fitlin(X,y);
   w2 = fitlinreg(X,y,lambda);
   w3 = fitgenreg(X,y,lambda,featurefun);

   yhat_train1 = X*w1;
   yhat_train2 = X*w2;
   yhat_train3 = predictgen(X,featurefun,w3);
